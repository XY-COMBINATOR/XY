...
- [x] Correct the actual deployed API-path/runtime defect in code: the active Supabase API no longer fails startup on an unused legacy JWT secret, while legacy cookie-session mode remains strictly validated. Added regression coverage; 51 tests, type-check, formatting, serverless compilation, and production build pass. Protected delivery remains pending.
