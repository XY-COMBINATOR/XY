...
const coordinates = [
  [
    ...
    "PIVOT",
    "The maker",
    "Builds the unexpected bridge between an idea and a working thing.",
  ],
  [
    "02",
    "FRAME",
    "The organizer",
    "Turns a loud set of possibilities into one calm, usable route.",
  ],
  [
    "03",
    "TRACE",
  ...
  [
    ...
    "VIVID",
    "The storyteller",
    "Gives each direction a visual pulse that is hard to forget.",
  ],
] as const;
...
export default function People() {
  ...
  const activeCoordinate =
    coordinates.find(coordinate => coordinate[1] === activeName) ??
    coordinates[0];
    ...
    <PublicFrame label="02 / PEOPLE">
      <main className="route-main ink-page people-page">
        <div className="people-heading">
          <p className="route-kicker light">THE FOUR COORDINATES</p>
          <h1>
            NO SINGLE
            <br />
            <i>POINT OF VIEW.</i>
        ...
        <section className="coordinate-list" aria-label="Team coordinates">
          {coordinates.map(([number, name, role, copy], index) => (
            <motion.button
              key={name}
              type="button"
              className={`coordinate-card card-${index + 1} ${activeName === name ? "is-active" : ""}`}
            ...
            >
              <span>{number}</span>
              <div className="coordinate-symbol" aria-hidden="true">
                <b />
                <i />
              </div>
              <p>{role}</p>
              <h2>{name}</h2>
              <div className="coordinate-copy">{copy}</div>
            </motion.button>
          ))}
        </section>
        <section className="coordinate-reader" aria-live="polite">
          ...
          <h2>
            {activeCoordinate[1]} — {activeCoordinate[2]}
          </h2>
