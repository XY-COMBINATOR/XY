...
function isOfflinePreviewEnabled() {
  return import.meta.env.DEV && new URLSearchParams(window.location.search).has("offlinePreview");
}
...
export function DashboardOfflineIndicator() {
  const [isOffline, setIsOffline] = useState(() =>
    isOfflinePreviewEnabled() || isNetworkOffline(typeof navigator === "undefined" ? undefined : navigator)
  );
  ...
  return (
    <aside className="dashboard-offline-indicator" role="alert" aria-live="assertive">
      <WifiOff size={17} aria-hidden="true" />
      <div>
        <strong>YOU’RE OFFLINE</strong>
        <span>Changes that need the server will wait until your connection returns.</span>
      </div>
    </aside>
  );
}
