...
const team = [
  ...
  },
  {
    ...
    role: "The connector",
    name: "TRACE",
    focus: "Finds the invisible patterns that let complex systems speak plainly.",
    tags: ["Systems", "Signal"],
    tone: "cobalt",
  },
  {
...
const capabilities = [
  ["01", "IDENTITIES", "Names, systems, and visual languages with a point of view."],
  ["02", "INTERFACES", "Digital experiences that make complexity feel inevitable."],
  ["03", "MOTION", "Movement with a job: clarity, energy, and a sense of arrival."],
  ["04", "PRODUCTS", "Practical tools made with editorial sharpness and engineering care."],
] as const;
    ...
    <div className={`xy-site ${isSoundOn ? "is-live" : ""}`}>
      ...
      <header className={`site-header ${scrolled ? "is-scrolled" : ""}`}>
        ...
        <div className="header-actions">
          <button className="sound-button" type="button" onClick={() => setIsSoundOn((value) => !value)} aria-label={isSoundOn ? "Disable ambient animation" : "Enable ambient animation"}>
      ...
      <main id="top">
        <section className="hero" aria-labelledby="hero-heading">
          ...
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            variants={{ visible: { transition: { staggerChildren: prefersReducedMotion ? 0 : 0.11 } } }}
          >
            <motion.p className="eyebrow light" variants={rise}>INDEPENDENT CREATIVE COLLECTIVE <Asterisk size={13} /></motion.p>
            <h1 id="hero-heading">
              <motion.span variants={rise}>DIFFERENT</motion.span>
              <motion.span variants={rise} className="hero-word-shift">COORDINATES.</motion.span>
              <motion.span variants={rise} className="hero-accent">ONE DIRECTION.</motion.span>
            </h1>
          ...
          <div className="hero-footnote">SCROLL TO COMBINE <ChevronDown size={16} /></div>
        </section>

        <section className="ticker" aria-label="XY COMBINATOR focus areas">
          <div className="ticker-track">
            {["IDENTITY", "INTERACTION", "CULTURE", "PRODUCT", "IDENTITY", "INTERACTION", "CULTURE", "PRODUCT"].map((item, index) => (
        ...
        <section id="about" className="about-section section-pad" aria-labelledby="about-heading">
          <motion.div className="section-label" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.4 }} variants={rise}>
            <span>02</span><Minus size={16} /><span>WHY WE COMBINE</span>
          </motion.div>
          <div className="about-layout">
            <motion.div className="about-statement" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={{ visible: { transition: { staggerChildren: prefersReducedMotion ? 0 : 0.08 } } }}>
              <motion.h2 variants={rise}>Good work doesn’t come from a single point of view.</motion.h2>
        ...
        <section id="team" className="team-section section-pad" aria-labelledby="team-heading">
          <div className="team-header">
            <motion.div className="section-label light" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={rise}>
          ...
          <div className="team-panel">
            <div className="team-axis" aria-hidden="true"><span>X</span><span>Y</span><b /></div>
              ...
              {team.map((member, index) => {
                ...
                return (
                  <motion.button
                    ...
                    transition={{ duration: 0.45, delay: prefersReducedMotion ? 0 : index * 0.08, ease: [0.23, 1, 0.32, 1] }}
                    aria-pressed={active}
                  >
                    <span className="member-number">{member.id}</span>
                    <span className="member-graphic" aria-hidden="true"><span /><span /></span>
                    <span className="member-role">{member.role}</span>
                    <span className="member-name">{member.name}</span>
                    <span className="member-plus">{active ? <Minus size={18} /> : <Plus size={18} />}</span>
                    <span className="member-detail">{member.focus}<em>{member.tags.join("  /  ")}</em></span>
                  </motion.button>
                );
              })}
            </div>
        ...
        <section id="capabilities" className="capabilities section-pad" aria-labelledby="capabilities-heading">
          <div className="capabilities-intro">
            <motion.div className="section-label" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={rise}>
              <span>04</span><Minus size={16} /><span>WHAT WE MAKE</span>
            </motion.div>
            <motion.h2 id="capabilities-heading" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.5 }} variants={rise}>Clarity with<br /><i>some voltage.</i></motion.h2>
          </div>
          <div className="capability-list">
            {capabilities.map(([number, title, description], index) => (
              <motion.article key={number} className="capability" initial={{ opacity: 0, x: prefersReducedMotion ? 0 : 25 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.45, delay: prefersReducedMotion ? 0 : index * 0.06 }}>
      ...
      <footer className="site-footer">
        <div className="footer-top"><span>XY COMBINATOR</span><span>ANYWHERE / EVERYWHERE</span><span>© 2026</span></div>
        <div className="footer-bottom"><p>Four points. One trajectory.</p><a href="mailto:hello@xycombinator.com">hello@xycombinator.com</a></div>
      </footer>
    </div>
  );
}
