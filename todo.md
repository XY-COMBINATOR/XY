...
- [ ] Correct the actual deployed API-path/runtime defect, add regression coverage, and deliver it through protected checks before requesting another live observation; the missing tRPC routes are delivered, but the live function currently returns 500 during invocation and requires the production runtime configuration to be verified.
...
- [ ] Replace the short 22-character Vercel `JWT_SECRET` with a new random value of at least 32 characters, redeploy, and verify the live tRPC function returns a non-500 response.
