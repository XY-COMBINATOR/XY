      ...
      <main className="route-main paper-page capability-page">
        ...
        <section className="capability-grid" aria-label="Capabilities">
          {work.map(([number, name, copy], index) => {
            ...
            return <motion.button key={name} type="button" className={`capability-choice ${active ? "is-active" : ""}`} onClick={() => setActiveCapability(name)} aria-pressed={active} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: index * 0.06 }}><span>{number}</span><em>{active ? "ACTIVE" : "OPEN"}</em><h2>{name}</h2><p>{copy}</p><div className="capability-extra">A focused working session maps the highest-value next decision before we design the visible output.</div></motion.button>;
          })}
        </section>
